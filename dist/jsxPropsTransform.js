...
function jsxPropsTransform({
  ...
  const transformBindAttr = (props, tag) => {
    ...
    return t.objectProperty(t.identifier(tag), t.arrowFunctionExpression([t.identifier("cb"), t.identifier("$bind"), t.identifier("Data")], t.arrowFunctionExpression([], t.callExpression(t.identifier(`cb`), [t.objectExpression(v.properties)]))));
  };
  return {
    ...
    visitor: {
      JSXElement: function (path) {
        if (path.node.openingElement && path.node.openingElement.name.name == "uc-template") {
          ...
          if (child && child.length > 0) {
            propAttrs.push(t.objectProperty(t.identifier("_children"), t.arrowFunctionExpression([t.identifier("props")], t.jsxFragment(t.jsxOpeningFragment(), t.jsxClosingFragment(), child))));
            path.node.openingElement.children = [];
          }
          let replacer;
          if (contentId) {
            if (isContentIdExpression) {
              replacer = t.callExpression(t.identifier(`loadPartialAsync`), [t.identifier(contentId), t.identifier("jsx"), t.objectExpression([t.spreadElement(t.identifier("props")), ...propAttrs]), t.identifier("Data")]);
            } else {
              replacer = t.callExpression(t.identifier(`_partialExtern[${contentId}]`), [t.identifier("jsx"), t.objectExpression([t.spreadElement(t.identifier("props")), ...propAttrs]), t.identifier("Data")]);
            }
      ...
      JSXOpeningElement(path) {
        const existingProps = path.node.attributes.filter(node => !node.name || node.name && ["attrs", "data", "_bind", "_listen", "ref", "class"].indexOf(node.name.name) < 0);
        const refs = path.node.attributes.find(node => node.name && node.name.name == "ref");
        if (refs && refs.value && refs.value.expression) {
          let insertHook = t.objectProperty(t.identifier("insert"), t.arrowFunctionExpression([t.identifier("vnode")], t.assignmentExpression("=", refs.value.expression, t.identifier("vnode.elm"))));
        ...
        path.node.attributes = path.node.attributes.filter(node => node.name && ["attrs", "on", "hook", "ref", "class"].indexOf(node.name.name) >= 0);
        let props = [];
        let on = [];
        if (path.node && path.node.name && path.node.name.name.startsWith("uc-")) {
          ...
          if (child && child.length > 0) {
            ...
            props.push(t.objectProperty(t.identifier("_children"), t.arrowFunctionExpression(cProps, t.blockStatement([t.variableDeclaration("const", [t.variableDeclarator(t.identifier("oldProps"), t.identifier("props"))]), t.blockStatement([t.variableDeclaration("const", [t.variableDeclarator(t.identifier("props"), t.objectExpression([t.spreadElement(t.identifier("oldProps")), t.spreadElement(t.identifier("_newProps"))]))]), t.returnStatement(t.jsxFragment(t.jsxOpeningFragment(), t.jsxClosingFragment(), child))])]))));
